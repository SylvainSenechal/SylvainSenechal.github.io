...
function predict(){
  tf.tidy( () => {
    ...
    let xs = tf.tensor4d(tab, [1, Partie.nbCase, Partie.nbCase, 1])
...
socket.on('dataSent', data =>{
  ...
  for(let record of data.entries){
    for(let i=0; i<Partie.nbCase; i++){
      for(let j=0; j<Partie.nbCase; j++){
        tab.push(record[1+i+j*Partie.nbCase])
      }
    }
  ...
  xs = tf.tensor4d(tab, [data.entries.length, Partie.nbCase, Partie.nbCase,1])
  ys = tf.oneHot(labelsTensor, 10)

  model = tf.sequential()
  let layer1 = tf.layers.conv2d({
    inputShape: [Partie.nbCase, Partie.nbCase, 1],
    kernelSize: 5,
    filters: 8,
    strides: 1,
    activation: 'relu',
